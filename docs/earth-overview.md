...
用户已确认的[火星 / 地球共用分析工作台](plans/2026-09-23-earth-shared-analysis-workbench.md)首期范围**已经实现**：三维地球、三栏工作台、年度分析、极区统计与图表 AI 解读见[共用分析工作台](earth-analysis-workbench.md)。本文仍是二维基础层的协议文档；页面现在默认挂载共用工作台，`EarthOverviewScene` 作为兼容保留的二维实现留在仓库中，二维地图与时间轴组件被工作台直接复用。

文档最近核对日期：**2026-09-24**；v2 数据核对见 [预处理记录](plans/2026-09-23-earth-reprocessing.md)，v1 历史验收见文末“验证记录”。

## 已实现范围

| 能力 | 状态 |
| --- | --- |
| 总览“火星 / 地球”切换（默认火星） | 已实现 |
...
| 三维全球球体、共用三栏工作台、年度分析、极区统计、图表 AI 解读 | 已实现，见 [共用分析工作台](earth-analysis-workbench.md) |
...
- 默认 `mars`；`planet=mars` 挂载原有火星内容，`planet=earth` **只**挂载地球场景（互斥挂载，不做 hidden 双挂载）。地球不挂载火星三维背景、摄像头、Mars 查询组件或 Copilot。
...
页面层现在默认挂载[共用分析工作台](earth-analysis-workbench.md)。以下约定对二维页面与三维工作台同时有效。

- 桌面布局在 70px 全局导航下方使用正常文档流，最大内容宽 1440px；≤900px 单列；390px 屏宽无横向溢出。
...
- 地球已开放三维日数据分析工作台（三维全球球体、年度分析、极区统计、图表 AI 解读），协议见[共用分析工作台](earth-analysis-workbench.md)。
...
- 二维实现（`EarthOverviewScene` 及其组件）作为兼容层保留：`EarthMap2D`、`EarthTimeline`、`EarthSeriesPanel` 被工作台直接复用，页面默认入口改为共用工作台。
...
- **真实浏览器验收**（系统 Edge + `playwright-core`，headless，1440/900/390px）：**43/43 项通过**，覆盖默认火星、切换地球、初始日期与 DU、1519 个数据格、闰日 `2020-02-29`、五变量原始单位、火星单位设置不改变地球数值、地图点选与标记、覆盖区外提示且不发点位请求、逐日播放不跳日、暂停与从首日重播、慢网下“正在加载所选日期”且不把旧值标成新日期、快速切换日期与变量后只保留最后选择、Earth→Mars→Earth 保留选择、中英文与深浅主题、无横向溢出、底图仅来自本地资源、控制台无新增错误。
